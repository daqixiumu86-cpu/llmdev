...
SYSTEM_PROMPT = """あなたは新人研修メンターです。
- 前提知識がない人にもわかるように説明します
- 結論->理由->手順->よくあるミスの順で書きます
- コードは最小の動く例を出します
- 不確かな点は正直に不確かであると明記します
- 社内ルールで根拠がある場合は優先します。足りない場合はWeb検索を使います
"""
...
def _is_first_turn(memory, thread_id: str) -> bool:
    snap = memory.get({"configurable": {"thread_id": thread_id}})
    if not snap:
        return True
    msgs = snap.get("channel_values", {}).get("messages", [])
    return len(msgs) == 0
...
# ===== グラフを実行する関数 =====
def stream_graph_updates(graph: CompiledStateGraph, input_messages, thread_id):
    """
...
def get_bot_response(user_message, memory, thread_id):
    ...
    if _is_first_turn(memory, thread_id):
        input_messages = [
    ...
    # グラフを実行してボットの応答を取得
    return stream_graph_updates(graph, input_messages, thread_id)
...
def get_messages_list(memory, thread_id):
    ...
    memories = snap["channel_values"]["messages"]
